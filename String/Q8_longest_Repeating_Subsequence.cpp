...
public:
    ...
    int longest_repeating_sub_App1(string str)
    {
        return solve1(str,str,0,0);
    }
    
    //Approach-2 DP
    int longest_repeating_sub_App2(string str)
    {
        //create 2d vector
        vector<vector<int>>dp(str.length(),vector<int>(str.length(),-1));
        return solve2(str,str,0,0,dp);
    }
};
...
int main()
{
    ...
    //Approach-1 recursion
    //cout<<q.longest_repeating_sub_App1(str);


    //Approach-2 using dp
    cout<<q.longest_repeating_sub_App2(str);
